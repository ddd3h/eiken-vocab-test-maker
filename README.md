...
PDFで書き出すツールです（GUI / CLI）。既定では左に問題・右に解答を1枚で作れます
（左右で別問題 or 同一問題2枚にも切り替え可能）。解答だけの別PDFも作れます。

単語帳は同梱の **英検2級 パス単（1700語）** / **ターゲット1900（1900語）** /
**英検準1級 パス単（1900語）** から選べるほか、自前のCSV（ファイル / URL）も使えます。
...
- **今すぐ更新**（ビルド済みアプリのみ）: その場で新版をダウンロード → SHA256 を検証 →
  アプリを入れ替えて自動的に再起動します。更新前に確認ダイアログが出ます。
  アプリ自身がダウンロードするため、更新後は macOS の「開発元を確認できません」警告も出なくなります。
- **ダウンロードページ**: 手動で更新したい場合や、ソースから実行している場合はこちら。

注意:

- macOS で、zip を展開した場所（ダウンロードフォルダなど）から直接起動した場合、
  macOS がアプリを一時的な場所で実行することがあり（App Translocation）、その状態では更新できません。
  一度 Finder でアプリを「アプリケーション」フォルダなどに移動してから起動してください。
- Windows は実行中のファイルを置き換えられないため、アプリ終了後に小さな PowerShell スクリプトが
  入れ替えと再起動を行います。
- 途中で失敗した場合は元のバージョンのまま残ります。
- オフラインでも普通に使えます（確認に失敗しても何も表示しません）。
  確認したくない場合は `--no-update-check` を付けて起動します。
- CLI: `--update`（置き換えまで実行）/ `--check-update`（確認だけ）
...
| オプション | 説明 |
|---|---|
| `--dataset` | 同梱の単語帳: `eiken2`（英検2級 パス単, 既定）/ `target1900`（ターゲット1900）/ `eikenpre1`（英検準1級 パス単） |
...
| `--update` | 新しいバージョンがあればダウンロードして置き換える（ビルド済みアプリのみ） |
| `--version` | バージョンを表示 |
...
- 単語帳: 英検2級 パス単（1700語）/ ターゲット1900（1900語）/ 英検準1級 パス単（1900語）/ カスタム（自前のCSV）
...
この形式のCSVを用意して「カスタム」で指定すれば、同梱の単語帳に限らず
汎用の単語テストメーカーとして使えます。
